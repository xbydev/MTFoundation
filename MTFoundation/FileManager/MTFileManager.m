...
+ (NSString *)createCacheFileDir:(NSString *)dirName
{
    ...
    cachePath = [documentPath stringByAppendingPathComponent:dirName];
...
+ (BOOL)clearCachesFromDirectoryPath:(NSString *)dirPath{
    //获得全部文件数组
    NSArray *fileAry =  [MTFileManager getAllFileNames:dirPath];
    //遍历数组
    BOOL flag = NO;
    for (NSString *fileName in fileAry) {
        NSString *filePath = [dirPath stringByAppendingPathComponent:fileName];
        flag = [MTFileManager clearCachesWithFilePath:filePath];
        if (!flag)
            break;
    }
    return flag;
