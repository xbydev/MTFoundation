...
#import "TextWaterMarkInfo.h"

static NSString *const kVideoPath = @"MTVideo";
...
- (NSString *)videoPath
{
    return [MTFileManager createCacheFileDir:kVideoPath];
}
#pragma mark == 获取视频帧
...
- (void)compositionVideoWithImage:(NSArray<UIImage *> *)images videoName:(NSString *)videoName success:(SuccessBlcok _Nullable)successBlcok
{
    ...
    NSDictionary *outPutSettingDic =[NSDictionary dictionaryWithObjectsAndKeys:AVVideoCodecH264,AVVideoCodecKey,
                                     [NSNumber numberWithInt:size.width],AVVideoWidthKey,
                                     [NSNumber numberWithInt:size.height],AVVideoHeightKey,nil];
    ...
    if (success) {
        ...
        [videoWriterInput requestMediaDataWhenReadyOnQueue:dispatchQueue usingBlock:^{
            while ([videoWriterInput isReadyForMoreMediaData])
            {
                ...
                NSInteger baseValue = 30;
                if (imageCount > 0) {
                    baseValue = (duration * 30)/imageCount;
                }
            ...
            dispatch_async(mainSerializationQueue, ^{
                ...
                if (success) {
                    ...
                    if (assetAudioTrack)
                    {
                        ...
                        //创建读出
                        AVAssetReader *assetReader = [[AVAssetReader alloc] initWithAsset:audioAsset error:&error];
                        CMTimeRange audioTimeRange = CMTimeRangeMake(kCMTimeZero, CMTimeMake(duration, 1));
                        assetReader.timeRange = audioTimeRange;
                        BOOL success = (assetReader != nil);
                        ...
                        if (success) {
                            NSError *outError;
                            assetWriter = [[AVAssetWriter alloc] initWithURL:[NSURL fileURLWithPath:outPutFilePath] fileType:AVFileTypeMPEG4 error:&outError];
                            success = (assetWriter != nil);
                        }
                        if (success) {
                            //将track里的数据 读取出来
                            ...
                            //将读取的内容写入
                            AVAssetWriterInput *assetWriterAudioInput = [AVAssetWriterInput assetWriterInputWithMediaType:[assetAudioTrack mediaType] outputSettings:compressionAudioSettings];
                            //                            [AVAssetWriterInput insertTimeRange:audioTimeRange ofTrack:assetAudioTrack atTime:kCMTimeZero error:nil];
                            if ([assetWriter canAddInput:assetWriterAudioInput]) {
                                [assetWriter addInput:assetWriterAudioInput];
                            }
                            ...
                            if (writeSuccess)
                            {
                                ...
                                if (videoWriterInput) {
                                    ...
                                    [videoWriterInput requestMediaDataWhenReadyOnQueue:dispatchQueue usingBlock:^{
                                        ...
                                        while ([videoWriterInput isReadyForMoreMediaData])
                                        {
                                            if (++index >= duration * 30) { //每秒30张图片，总共有 duration * 30 张图片
                                                [videoWriterInput markAsFinished];
                                                dispatch_group_leave(dispatchGroup);
                                                break;
                                            }
...
- (void)addTextWatermarks:(NSArray *)textWatermarks ForVideo:(NSURL *)videoUrl videoName:(NSString *)videoName success:(SuccessBlcok)successBlcok{
...
//    [self addWaterLayerWithAVMutableVideoComposition:mutableVideoComposition];
    [self addWaterLayerWithAVMutableVideoComposition:mutableVideoComposition withTextInfo:textWatermarks];
...
- (void)addWaterLayerWithAVMutableVideoComposition:(AVMutableVideoComposition*)mutableVideoComposition withTextInfo:(NSArray*)textWatermarks
{
    CALayer *parentLayer = [CALayer layer];
    CALayer *videoLayer = [CALayer layer];
